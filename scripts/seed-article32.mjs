/**
 * Seed script: Insert Article 32 — 福岡ラーメン・もつ鍋巡り
 * Run: node scripts/seed-article32.mjs
 */
...
const { articles, articleTranslations } = schema;

// ── 1. Create the article record ──────────────────────────────────────────────
const [existing] = await db
  .select()
  .from(articles)
  .where(eq(articles.slug, "fukuoka-ramen-motsu-nabe-guide"))
  .limit(1);

let articleId;
if (existing) {
  articleId = existing.id;
  console.log(`Article already exists (id=${articleId}), updating...`);
  await db
    .update(articles)
    .set({
      status: "published",
      thumbnailUrl: "/manus-storage/ramen-shop-lantern_58d579b1.jpg",
      publishedAt: new Date(),
    })
    .where(eq(articles.id, articleId));
} else {
  const [result] = await db.insert(articles).values({
    slug: "fukuoka-ramen-motsu-nabe-guide",
    categoryId: 2, // gourmet
    schemaType: "Article",
    status: "published",
    thumbnailUrl: "/manus-storage/ramen-shop-lantern_58d579b1.jpg",
    publishedAt: new Date(),
...
<img src="/manus-storage/ramen-shop-night-tokyo_f041b974.jpg" alt="夜の東京ラーメン街の賑わい" style="width:100%;border-radius:8px;margin:1.5rem 0;" />
<p style="font-size:0.75rem;color:#aaa;text-align:right;margin-top:-0.75rem;">Photo by Pexels / Free License</p>
...
<div style="background:#F7F7F7;border-left:4px solid #000000;padding:1.25rem 1.5rem;border-radius:0 4px 4px 0;margin:2rem 0;">
  ...
  <a href="https://yah.mobile" target="_blank" rel="noopener" style="display:inline-block;margin-top:0.75rem;background:#000000;color:#fff;padding:0.5rem 1.25rem;border-radius:2px;text-decoration:none;font-weight:600;letter-spacing:0.04em;">yah.mobileを見る →</a>
</div>
...
const { articleTranslations: at } = schema;

const [existingTrans] = await db
  .select()
  .from(at)
  .where(eq(at.articleId, articleId))
  .limit(1);

if (existingTrans) {
  await db
    .update(at)
    .set({ title, excerpt, body, metaTitle, metaDescription, lang: "ja" })
    .where(eq(at.id, existingTrans.id));
  console.log("Translation updated.");
} else {
  await db.insert(at).values({
    articleId,
    lang: "ja",
    title,
    excerpt,
...
console.log(`\n✅ Article 32 seeded successfully!`);
console.log(`   ID: ${articleId}`);
console.log(`   Slug: fukuoka-ramen-motsu-nabe-guide`);
console.log(`   URL: /articles/fukuoka-ramen-motsu-nabe-guide`);
