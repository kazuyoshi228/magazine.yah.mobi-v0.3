 ...
 * Priority: Manus Forge (dev) → OPENAI_BASE_URL (production)
 */
const resolveApiUrl = () => {
...
/**
 * Resolve the API key.
 * Priority: Manus Forge key (dev) → OPENAI_API_KEY (production)
 */
const resolveApiKey = (): string => {
  ...
  throw new Error(
    "LLM API key is not configured. Set BUILT_IN_FORGE_API_KEY (dev) or OPENAI_API_KEY (production)."
  );
};

const assertApiKey = () => resolveApiKey(); // kept for backward compat
